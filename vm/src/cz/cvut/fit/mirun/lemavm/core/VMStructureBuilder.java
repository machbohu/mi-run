...
public class VMStructureBuilder {
	private static final Logger LOG = Logger
			.getLogger(VMStructureBuilder.class);
	...
	/**
	 * Print tree graph to console in standardized .dot format on-linux$ dot
	 * -Tpng -o output.png input.dot
	 */
					...
					if (child.toString().equals("VAR_DECLARATOR")
						...
						throw new VMParsingException(
								"Unexpected program syntax '"
										+ child.toString() + "' in class '"
										+ cls.getName() + "'");
					}
					cls.addField(new VMField(name, isStatic, visibility, type,
							val));
				}
				break;
			default:
				throw new VMParsingException("Unexpected program syntax '"
						+ child.toString() + "' in class '" + cls.getName()
						+ "'");
			}
		}
	}
	...
	private void buildMethodFromTree(CommonTree node, VMClass cls) {
		...
		VMMethod method = new VMMethod(name, cls, isStatic, visibility,
				returnType, args, block);
		cls.addMethod(method);
	}

	/**
	 * Read structure from given node and build variables, constructors and
	 * methods for given class
	 * 
	 * @param node
	 * @param cls
	 */
	...
	private void buildClassFromTree(CommonTree node) {
		...
		for (Object o : node.getChildren()) {
			...
			switch (child.toString()) {
			case "MODIFIER_LIST":
				// TODO if has more than two children, throw exception? (i.e.
				// public, static)
				// TODO All classes will be public and non-static
				if (child.getChildCount() > 0) {
					// visibility =
					// VMVisibilityModifier.fromString(child.getChild(0).toString());
					// isStatic = (child.getChildren().indexOf("static") != -1);
				}
				break;
	...
	/**
	 * Build Meta classes, their variables, constructors and methods from tree
	 * structure
	 */
